...
function Header() {
  ...
  return (
    <header className='flex fixed top-0 w-full justify-between z-10'>
      <div className='p-2 w-33'>
        <Link to='/'>
          <svg xmlns="http://www.w3.org/2000/svg" className='w-full fill-white' viewBox="0 0 103 44">
      ...
      >
        ...
        <div className='mr-9 flex justify-end flex-col items-end gap-[4px] z-2'>
          <div className='w-13 h-[2px] bg-white group-hover:bg-black'></div>
          <div className='w-6 h-[2px] bg-white group-hover:bg-black'></div>
        </div>
      </div>

      {menu && <FullWidthNav menu={menu} onClose={() => setMenu(false)} />}
    </header>
  )
}
