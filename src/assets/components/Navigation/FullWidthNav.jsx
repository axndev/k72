import React, { useState, useRef, useEffect } from "react";
import { Link } from "react-router";
import { useGSAP } from "@gsap/react";
import gsap from "gsap";

function FullWidthNav({ onClose }) {
    const stairsDiv = useRef(null);
    const links = useRef(null);
    const tl = useRef(null); // GSAP timeline reference

    // Setup animation
    useGSAP(() => {
        tl.current = gsap.timeline({ paused: true });

        tl.current
            .to(stairsDiv.current, {
                display: "block",
                duration: 0,
            })
            .from(".stair", {
                height: 0,
                stagger: -0.1,
                duration: 0.4,
            })
            .from('.closeBtn', {
                opacity: 0,
                duration: 0.1
            })
            .from(".link", {
                opacity: 0,
                rotateX: 90,
                transformOrigin: "top center",
                y: 30,
                stagger: 0.1,
                duration: 0.6,
                ease: "back.out(1.7)",
            });
    });

    // Function to close nav with animation
    const handleClose = () => {
        if (tl.current) {
            tl.current.reverse(); // Play the timeline backward
            tl.current.eventCallback("onReverseComplete", () => {
                onClose?.(); // Call parent’s close handler (optional)
            });
        }
    };

    const navClose = () => {
        if (tl.current) {
            const navHeader = document.getElementById('navHeader');
            navHeader.style.display = 'none'
            onClose();
        }
    }
    // Automatically play open animation when mounted
    useEffect(() => {
        if (tl.current) tl.current.play();
    }, []);

    return (
        <div id='navHeader' className="overflow-hidden fixed top-0 left-0 h-screen w-full text-white flex flex-col justify-center items-center z-15">
            <div ref={stairsDiv} className="h-screen w-full fixed top-0">
                <div className="h-full flex w-full">
                    {[...Array(5)].map((_, i) => (
                        <div key={i} className="stair h-full w-1/5 bg-black"></div>
                    ))}
                </div>
            </div>
            <div className="w-full">
                {/* Top Bar */}
                <div className="headerLogo p-2 flex fixed top-0 w-full justify-between items-center z-10">
                    <div onClick={navClose} className="w-29 -mt-18">
                        <Link to="/">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                className="w-full fill-white"
                                viewBox="0 0 103 44"
                            >
                                <path d="M35.1441047,8.4486911 L58.6905011,8.4486911 L58.6905011,-1.3094819e-14 L35.1441047,-1.3094819e-14 L35.1441047,8.4486911 Z M20.0019577,0.000230366492 L8.83414254,25.3433089 L18.4876971,25.3433089 L29.5733875,0.000230366492 L20.0019577,0.000230366492 Z M72.5255345,0.000691099476 L72.5255345,8.44846073 L94.3991559,8.44846073 L94.3991559,16.8932356 L72.5275991,16.8932356 L72.5275991,19.5237906 L72.5255345,19.5237906 L72.5255345,43.9274346 L102.80937,43.9274346 L102.80937,35.4798953 L80.9357483,35.4798953 L80.9357483,25.3437696 L94.3996147,25.3428482 L94.3996147,16.8953089 L102.80937,16.8953089 L102.80937,0.000691099476 L72.5255345,0.000691099476 Z M-1.30398043e-14,43.9278953 L8.78642762,43.9278953 L8.78642762,0.0057591623 L-1.30398043e-14,0.0057591623 L-1.30398043e-14,43.9278953 Z M58.6849955,8.4486911 L43.1186904,43.9274346 L52.3166592,43.9274346 L67.9877996,8.4486911 L58.6849955,8.4486911 Z M18.4688864,25.3437696 L26.7045278,43.9278953 L36.2761871,43.9278953 L28.1676325,25.3375497 L18.4688864,25.3437696 Z"></path>
                            </svg>
                        </Link>
                    </div>

                    {/* Close Button */}
                    <div
                        onClick={handleClose}
                        className="closeBtn flex justify-center items-center h-30 w-30 relative pr-3 cursor-pointer group"
                    >
                        <div className="h-36 w-[2px] bg-white rotate-45 group-hover:bg-[#D3FD50] absolute"></div>
                        <div className="h-36 w-[2px] bg-white -rotate-45 group-hover:bg-[#D3FD50] absolute"></div>
                    </div>
                </div>

                {/* Nav Links */}
                <div
                    ref={links}
                    className="flex flex-col justify-center items-center w-full text-center"
                >
                    {["Projects", "Agence", "Contact", "Blog"].map((label, i) => (
                        <div
                            onClick={navClose}
                            key={i}
                            className={`link origin-top text-[7.5vw] uppercase font-[font2] border-t w-full group border-white leading-[8vw] -mb-3 relative ${label === "Blog" ? "border-b leading-[85px] pt-2" : ""
                                }`}
                        >
                            <Link to={`/${label.toLowerCase()}`} className="-mb-4">
                                {label}
                                <div className={`flex gap-5 overflow-hidden absolute top-0 bg-[#D3FD50] h-0 group-hover:h-[90%] opacity-0 group-hover:opacity-100 transition-all duration-200 text-black ${label === "Blog" ? "group-hover:h-full" : ""}`}>
                                    {[...Array(2)].map((_, j) => (
                                        <div
                                            key={j}
                                            className="flex moveX shrink-0 items-center gap-3"
                                        >
                                            <span className="leading-[8vw] -mb-3">Pour tout voir</span>
                                            <img
                                                className="w-65 h-22 object-cover rounded-full"
                                                src="../../public/images/header-img.jpg"
                                                alt=""
                                            />
                                        </div>
                                    ))}
                                </div>
                            </Link>
                        </div>
                    ))}
                </div>
            </div>
        </div>
    );
}

export default FullWidthNav;
